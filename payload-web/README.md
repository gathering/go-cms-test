...
Steps require working docker and docker-compose (there is also local setup instructions available in main docs)

1. Copy `.env.example` to `.env`
2. Run `docker-compose up`. This might take a good while the first time.
3. Visit `http://localhost:3000/admin` to create the first admin user (fake mail works)
4. Visit `http://localhost:3000/api/seed` (or run `yarn seed` inside web container) to add seed data
5. Visit `http://localhost:3000` and play around

## Misc tips

- Payload config and files for the demo site lives under `./src/payload` - Try and modify the code for it
- Code under `./src/app` are only frontend code that uses Next.js - Not part of CMS
- To clean up once done testing (including deleting database, run `docker-compose down`)
- To run any yarn commands inside the web container use something like `docker-compose exec web yarn <the command>` while containers are running
- [Installation documentation](https://payloadcms.com/docs/getting-started/installation) for examples off what a minimal setup involves
- Demo uses MongoDB, and a the just released v2. Which in theory adds support for Postgres and migrations. Unable to get working during local setup (encountered https://github.com/payloadcms/payload/pull/3666 and others). Hopefully we can use it once we get to production.
